...
export default function sortOption(chartData: any[] = []) {
  ...
  // 分离排序后的数据
  const salvProName = sortedData.map((item) => item.name)
  const data: any = sortedData.map((item) => item.value)

  const dataBg = Array(data.length).fill(1)
  const maxData = Math.max(...data) * 1.2
  const myColor = ['#1edcaa', '#00daed'].reverse()
    ...
    yAxis: [
      {
        ...
        axisLabel: {
          ...
          formatter: (value: any, index: any) => {
            const num = index + 1
            return `{num|NO.${num}}{nameStyle|${value}}`
          },
          rich: {
            num: {
              color: '#fff',
      ...
      },
      {
        ...
        axisLabel: {
          ...
          verticalAlign: 'top', // 顶部对齐[6](@ref)
          align: 'right', // 右对齐[6](@ref)
          padding: [-20, 0, 30, 20], // 左内边距微调
